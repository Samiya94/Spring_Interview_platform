...
function addDept(){
  ...
  if(!n){showToast('Enter a department name.','warn');return;}
  if(departments.find(d=>d.name.toLowerCase()===n.toLowerCase())){showToast(`"${n}" already exists.`,'warn');return;}
  departments.push({name:n,coordinator:'Not Assigned'});
  saveDepts(); renderSettingsTable(); renderDeptCards(); renderDeptCheckboxes(); renderOverview(); renderReqStats();
  inp.value=''; showToast(`Department "${n}" added.`);
}
...
function confirmDeleteDept(){
  ...
  const n=departments[_delIdx]?.name;
  departments.splice(_delIdx,1);
  _delIdx=null;
  if(n){
    const leg=getLegacy().filter(d=>(d.departmentName||d.name)!==n);
    localStorage.setItem('departments',JSON.stringify(leg));
    localStorage.removeItem('reqStatus_'+n);
  }
  saveDepts(); renderSettingsTable(); renderDeptCards(); renderDeptCheckboxes(); renderOverview(); renderReqStats();
...
/* ═══════════════ REGISTRATION LINK ═══════════════ */
function genRegLink(){
  let link=localStorage.getItem('registrationLink_'+iid);
  if(!link){
    const tok='REG-'+Math.random().toString(36).substring(2,10).toUpperCase();
    link=location.origin+'/tpo-coordinator-registration.html?inst='+iid+'&token='+tok;
    localStorage.setItem('registrationLink_'+iid,link);
  }
  localStorage.setItem('registrationBaseOrigin_'+iid,location.origin);
